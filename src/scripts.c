LSGFW_API i32_t lsgfw_attach_scripts(u32_t world_i, const char* path, const char* pattern)
{
	lsgfw_glob_t script_glob = NULL;
	if (!lsgfw_glob(&script_glob, path, pattern))
...
{
	void* (**script_funcs)(lsgfw_universe_t*, u32_t) = ((void* (***)(lsgfw_universe_t*, u32_t)) (&universe.world_v[world_i].scripts))[func_offset];
